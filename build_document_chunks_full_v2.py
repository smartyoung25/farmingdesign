...
#        "수현건**설임미라**님" → 임미라. 근거: 이 말뭉치의 확인된 사람 이름이
#        **전원 3글자**다 — `CASE_OVERRIDES`의 인명 **16명** + 폴더 기반 케이스
#        (이녕연·최혁진·이준호) = **19명** 전부(104차 레드팀 F7로 "22명" 오기 정정).
#        유일한 5자 `한일그린텍`은 사람이 아니라 업체 case명이다.
...
#   ⚠️ **취약점은 양방향이다**(104차 레드팀 F8로 후자 추가):
#      ①새 지명·업체명이 들어오면 `NOT_A_NAME`에 없어 다시 오탐한다.
#      ②**반대로 사람 이름이 `NOT_A_NAME`과 충돌할 수 있다** — `수현·서진·다온·
#        미래·그린·백두`는 2글자 이름으로 존재할 수 있고, 그러면 `([가-힣]{2,3})농가`
#        등이 2글자로 백트래킹해 매치→거부→**영구 미상**이 된다. 현재 말뭉치의
#        케이스명 19개와는 교집합이 공집합이라 발생하지 않는다(테스트로 고정).
NOT_A_NAME = {
    ...
    "비닐", "온실", "하우스", "자재", "관리", "작업",
    # ⚠️104차 레드팀 F8: "비닐온실"·"스마트팜"은 4글자라 {2,3} 캡처와 절대 매치되지
    #   않는 **사문 항목**이었다 — 제거했다(그 오탐은 {2,4}였던 시제품에서만 났다).
}
