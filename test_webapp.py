...
def test_215cha_notice_edition_is_named_not_assumed():
    ...
    **모순이 아니라 시점 차이**다. 벤치마킹 문서도 지침도 틀리지 않았다.
    ...
    쓸지는 **★사용자 결정**(대장 D-15)이고 엔진은 **고르지 않는다** — 드러낼 뿐이다.
    """
    import smartfarm_engine as _e
    doc = _io_read("근거_외부수집_기능공백_20260924.md")
...
def test_216cha_edition_diff_is_recounted_not_asserted():
    ...
    moved_w = [(n, v, cur[n]["wind_ms"]) for n, v in WIND_2022.items()
               if cur[n]["wind_ms"] != v]
    assert len(moved_s) == 14 and len(moved_w) == 8, (
        f"🔴 두 판 차이가 적설 {len(moved_s)} · 풍속 {len(moved_w)}로 바뀌었다 — "
        "216차 실측은 **14 · 8**이다. 설계하중 표를 손댔다면 **어느 판 기준인지**를 "
        "먼저 정해야 한다(★대기 D-15)")

    # 🔴 방향이 한쪽인가 — 하나라도 2025판이 작아지면 성격이 달라진다
    ...
    assert d["snow_regions"] == len(moved_s) and d["wind_regions"] == len(moved_w), (
        "🔴 엔진 주석의 차이 수가 **다시 센 값과 다르다** — 주석이 썩었다")
    ...
    for c in _lc():
        ...
        assert not [n for n in names if n in rg], (
            ...
            "D-15가 실제 영향을 갖게 됐으니 ★결정을 먼저 받아야 한다")

    # ── ⚠️ D-15가 **닫힌 척하지 않는가** ───────────────────────────
    assert "닫히지 않고 좁혀졌다" in ledger, (
        "🔴 D-15가 **닫힌 것처럼** 적혔다 — 「어디서 얼마나」는 알아도 「어느 판을 "
        "준거로」는 여전히 ★사용자 결정이다")
...
def test_218cha_middle_edition_is_opened_too():
    ...
    for nm, v in SNOW_2019_DIFF.items():
        ...
        assert R[nm]["snow_cm"] > v, (
            f"🔴 `{nm}` 적설이 2019 {v} → 2025 {R[nm]['snow_cm']}로 **커지지 "
            "않았다**. 218차 실측은 **세 대조 모두 뒤 판이 크거나 같다**였다")
...
def test_219cha_all_ten_editions_were_opened():
    ...
    **별표가 없다**. 2014-78호는 표를 고쳤다고 **스스로 적는데** 그 표가 없다.
    ...
    assert "제2016-180호 별표는 제2019-44호와 완전히 같다" in eng, (
        "🔴 엔진 주석에서 *「2016-180 = 2019-44」*가 사라졌다 — 이것이 "
        "**값이 움직인 지점은 둘뿐**이라는 결론의 근거다")
    ...
    for nm, v in SNOW_2016_2019.items():
        assert R[nm]["snow_cm"] > v, (
            f"🔴 `{nm}`이 2016/2019판 {v}보다 크지 않다 — 두 판이 같다는 전제가 "
            "깨지거나 엔진 값이 움직였다")
