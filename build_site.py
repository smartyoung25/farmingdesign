...
def capex_breakdown_page() -> str:
  ...
  <section class="card"><span class="axis">총사업비 관점 · 13개 상위 카테고리</span>
    ...
    <p class="note">⚠️ 케이스 간 비중 비교 시 분모(known_total) 구성이 문서 구조에 따라 다르다 —
      ...
      (산업안전보건관리비 1,440,000원)·이준희(산업안전 27,047,871원+환경보전 5,051,685원=32,099,556원)는
      같은 성격 비용이 원가계산서 산식 항목이라 분모 밖이다(레드팀 4·5회차 관찰 — 비중의 한 자리
      수준 차이는 이 구조 차이만으로도 생길 수 있다).</p>
  </section>
