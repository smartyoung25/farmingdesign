...
# 파일명만으로 doc_type을 못 잡는 소수 사례에 대한 수동 예외(파일럿에서 내용 확인 완료).
# 정규식을 억지로 넓히면 다른 파일 오탐이 늘어나므로, 이미 근거(내용 확인)가 있는 개별
# 파일만 명시적으로 지정한다 — "근거 없이 채우지 않는다" 원칙과 같은 이유로 최소화한다.
DOC_TYPE_OVERRIDE = {
    "스마트팜스펙/최혁진님 온실 내역서/스마트팜 신축공사_최혁진.pdf": "설계도면",
    "스마트팜스펙/최혁진님 온실 내역서/스마트팜 신축공사_최혁진_OCR.pdf": "설계도면",
}


def find_non_ocr_sibling(path):
    """path가 *_OCR.ext 형태면 원본(비-OCR) 형제 파일 경로를 반환, 아니면 None."""
    d, base = os.path.split(path)
    stem, ext = os.path.splitext(base)
    if not stem.endswith("_OCR"):
        return None
    orig_stem = stem[: -len("_OCR")]
    candidates = glob.glob(os.path.join(d, f"{orig_stem}{ext}"))
...
def classify(path):
    ...
    if rel in DOC_TYPE_OVERRIDE:
        # case는 아래 로직으로 정상 산출하고 doc_type만 override
        pass
    ...
    doc_type = DOC_TYPE_OVERRIDE.get(rel) or guess_doc_type(basename, rel_dir)
        ...
        if ext == ".pdf":
            if doc_type in PAGE_LEVEL_TYPES:
                ...
                if is_scan_only(path):
                    ocr = find_ocr_sibling(path)
                    if ocr:
                        nt, nl = chunk_pdf_table_or_lines(W, ocr, case, doc_type, None)
                        log.append(f"[{case}] {doc_type} {os.path.basename(path)}: 스캔본 → OCR본 사용 (table_pages={nt}, line_pages={nl})")
                    else:
                        W.skip(path, "스캔본이고 OCR본 없음 — 건너뜀(추정 금지)")
                        log.append(f"[{case}] {doc_type} {os.path.basename(path)}: 스캔본, OCR본 없음 → 건너뜀")
                else:
...
PROGRESS_LOG = os.path.join(ROOT, "_chunk_full_progress.log")
...
def run():
    ...
    progress_f = open(PROGRESS_LOG, "w", encoding="utf-8")
    progress_f.write(f"총 {len(all_files)}개 파일 처리 시작\n")
    progress_f.flush()

    for idx, path in enumerate(sorted(all_files), start=1):
        progress_f.write(f"[{idx}/{len(all_files)}] {os.path.basename(path)}\n")
        progress_f.flush()
        basename = os.path.basename(path)
        ext = os.path.splitext(path)[1].lower()
        ...
        if ext == ".pdf":
            sibling = find_non_ocr_sibling(path)
            if sibling and not is_scan_only(sibling):
                W.skip(path, f"원본({os.path.basename(sibling)})에 이미 텍스트 있음 — OCR본 중복 처리 방지")
                continue
...
write_outputs(
    W,
    os.path.join(ROOT, "문서청킹_인덱스_전체.jsonl"),
    os.path.join(ROOT, "문서청킹_전체_요약.txt"),
    extra_log=log,
)
