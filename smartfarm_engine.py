...
#   🔴**218차 정정 — 「한 세대」가 아니라 「두 세대」였다**: 함평 적설심은 제2014-78호 36 · **제2019-44호 36** · **제2022-104호 「40 이상」** · 제2025-108호 40이다.
#   🔴**220차 재확인**: 농사로에서 2014-78호 원본(619쪽 PDF)을 다시 받아
#   **셀 경계로** 읽은 결과 함평 36이 맞았다 — 과거 전사는 정확했다.
#   그리고 **2014 = 2016 = 2019**다(343항목 차이 0).
#   즉 2025년 구조계산서의 40은 **2022판을 따른 것**이지 2025 개정을 미리 본 것이
#   아니고, 2026-07 발견 시점에 우리 출처는 **2022·2025 두 판** 뒤처져 있었다..
...
# ─────────────────────────────────────────────────────────────
#   🔴**215차 — 판본 계보를 원문 부칙에서 다시 셌다**(사용자 지시: 이견 ⑤ 닫기).
#   위 서술은 2019-44호를 *「한 세대 전」*이라 적었는데, 고시 원문의 부칙 목록은
#   네 판을 보여 준다 — **<제2016-180호, 2016.12.28> · <제2019-44호, 2019.8.23> ·
#   <제2022-104호, 2022.9.30> · <제2025-108호, 2025.10.31>**. 즉 2019-44호와
#   2025-108호 사이에 **2022-104호가 한 판 더 있었다**(우리가 몰랐다).
#   ⚠️ **값은 바꾸지 않는다** — REGION_DESIGN_LOAD는 **최신 2025-108호** 전면
#      교체본 그대로다. 고친 것은 **계보 기록**뿐이다.
#   🔴**그래서 주의할 것이 하나 생겼다**: `2025년 온실신축 사업시행지침`은
#      **제2022-104호**를 참조한다(214차 원문 확인, 7회 전부). 그 지침으로 진행하는
#      사업의 **규격 설계도 준거**는 2022-104호이고, 이 엔진의 설계하중은
#      **2025-108호**다. 두 판의 값이 다른 지역이 있으면(함평 36→40cm 선례)
#      **지침 준거와 엔진 값이 어긋날 수 있다**.
#      ⚠️어느 판을 쓸지는 **★사용자 결정**이다 — 엔진은 고르지 않고 **드러낸다**.
#      📌`REGION_DESIGN_LOAD_EDITION`이 어느 판인지 이름으로 말한다.
...
REGION_DESIGN_LOAD_EDITION = {
    ...
    "note": ("⚠️`2025년 스마트팜 ICT 융복합확산(온실신축) 사업시행지침`은 "
             ...
             "아니다. 🔴217차에 `고성`·`광주` 2건도 닫아 **대조 사각 0**이다"
             "(열을 살려 읽고 같은 셀의 이웃으로 갈랐다 — 새 차이 없음). "
             "⚠️현재 케이스·견적 지역은 **22지역에 들지 않는다**. "
             "🔴218차 3판 대조: **2019-44↔2025는 72건**(적설 34·풍속 38)이고 "
             "2019-44↔2022-104는 50건이다 — **한 번도 값이 내려간 적이 없다**. "
             "🔴220차: **제2014-78호 = 제2016-180호 = 제2019-44호**(343항목 차이 0) — 8년(2014~2022) 동안 표가 그대로였고 값이 움직인 지점은 **2022·2025 두 번뿐**이다. "),
    "diff_2022_vs_2025": {"snow_regions": 14, "wind_regions": 8,
                          "uncompared": (),  # 🔴217차에 0이 됐다
                          "compared_regions": 170,
                          "direction": "2025판이 크다"},
}
...
# "0103.철골공사" 실측 163,468원/평과 오차 0.04%로 거의 정확히 일치 확인.
#   🔴146차(레드팀 27회차 [1]) 정정: 앞서 "÷702.36평=163,470"이라 적었는데 **두 면적
#     계열을 섞은 식**이었다. 702.36평은 2,321.87㎡(내역서 수량 계열)이고 163,470·오차
#     0.04%는 2,323㎡(표지 "=2,323m2")에서 나온다. 표지 기준으로 통일 —
#     2,323㎡=702.7075평, 114,870,017÷702.7075=163,467.75, 오차 +0.0415%.
#     (2,321.87을 쓰면 702.3657평·163,547.31원/평·오차 +0.0902%다 → D-7의 감응.)
...
CAPEX_MAJOR_EVIDENCE_STATUS = {
    ...
    "auxiliary_facility": "실측(1건: 윤성호 '0116 관리사(사무동)공사' 40,093,200원 — 독립 공종, 2026-08-17 P2-17) — 🔴142차 보강: **나머지 14건이 0인 것은 '부대시설이 없다'는 뜻이 아니다**. 리포가 이미 기록한 사유가 3건 있다 — **이준희**는 재배실 4,756.32+**관리실 648㎡**가 실재하나 '0402 재배실벽체 및 **관리동**지붕공사'가 온실구조에 **혼입**돼 0(52차) · **강정구**는 부속자재에 **관리동 판넬 및 출입문 부속 5,597,384원**(146차 재집계 식 병기 — 판넬·출입문 7줄 합)이 포함되나 **독립 공종이 아니라** 분리하지 않음(56차) · **오기수**는 **관리동커텐 2,817,269원**이 자동개폐로 귀속(57차). ⚠️**나머지 11건의 0 사유는 확인되지 않았다** — 원문 재확인 대상이고 **판정하지 않는다**. 🔴143차 전수 점검: 견적 원문 12파일을 직접 열어 보니 **0인 14건 중 13건에 관리동·작업동 계열이 실재**한다 — 최혁진 **관리동 트러스 상현재/하현재**(각 1,020,144원)·관리동전후면 샛기둥 851,059원 · 이두희 **작업동 콘크리트 121.20㎥ 11,887,296원** · 한일그린텍 **트러스(작업장) 1,406,856원** · 맹주연 작업장 마감파이프·농업용스크린(작업장포함) 12,125,520원 · 백가은·조윤정 **콘크리트 양액실/관리실 49㎡ 6,370,000원** · 박규현 공종명 자체가 *\"수평커튼(재배동:2구역2중)+(**작업동**:AL고정스크린)\"* · 구창회 **트러스(작업장) 1,985,596원** · 한수진 **작업동바닥콘크리트타설 9m×42m 1,150,000원** · 최선동 **작업동바닥콘크리트 9m×35m 1,250,000원·관리동 비닐 240,500원** · 우민재는 내역서엔 키워드 0건이나 **공사설명서 p0 면적표와 p1 '작업장 측면 단열 샌드위치패널'**에 실재. 🔴**그래도 0이 틀린 것은 아니다** — 찾은 항목은 거의 전부 **골조·기초·커튼·피복 공종 안의 라인**이라 분류 규칙(독립 공종만 편입)상 0이 맞다. ⚠️146차 정정(레드팀 27회차 [7]): 142·143차가 적은 *「독립 부대시설 공종은 윤성호 0116 하나뿐」*은 **틀렸다** — 오기수 집계표 p2에 공종번호가 붙은 최상위 행 「3. 관리동커텐공사 식 1 2,817,269」가 실재한다(원문 재확인). 오기수의 0 사유는 「독립 공종이 아니어서」가 아니라 **커튼 공종이라 보온커튼 관례(57차)로 자동개폐에 귀속**했기 때문이다 (엔진 주석 2713·2727이 처음부터 그렇게 적고 있었다). 즉 **14건의 0은 한 규칙으로 설명되지 않는다** — 혼입·공종 성격·미확인이 섞여 있다. **문제는 값이 아니라 읽힘이다**. ⚠️**임미라만 확인되지 않았다** — 전 시트 471행·키워드 20종으로 찾았으나 0건이고, **이 표본은 리포에 견적서 1종뿐**(도면·설명서 없음)이라 **\"없다\"고 말할 수 없다**(우민재가 바로 그 경우였다 — 내역서 0건인데 설명서에 있었다). ⚠️**금액을 이 카테고리로 옮기지 않았다** — 옮기면 분류 규칙이 깨지므로 **★사용자 결정**이다. 상세: `근거_관리동_실재점검_20260915.md`",
    "thermal_storage_insulation": "부분실측(축열탱크만 2건: 윤성호 축열조 90ton 41,958,000원[2026-08-17 P2-17]·임미라 축열조탱크 FRP보온 20톤 11,000,000원[91차 — 히트펌프 블록 내 라인분리, 윤성호와 동일 방식] 라인분리. 보온커튼은 이 카테고리가 아니라 자동개폐 시스템 소속 — 2026-08-17 사용자 결정으로 분류경계 확정(우민재 방식 통일). 이동혁(공주) 다겹보온커튼 분리견적 59,948,820원/1,139평은 자동개폐 계열의 커튼 시스템 일체 참고단가로만 기록. 이 카테고리의 잔여 공백은 단열판·기타 고정 단열재 실측뿐) — 🔴142차 보강: **나머지 13건이 0인 범위는 '축열탱크'에 한정**된다(보온커튼은 위와 같이 자동개폐 소속이고 단열판 등 고정 단열재는 이 카테고리의 잔여 공백이다). ⚠️**13건에 축열탱크가 실재하는지는 확인되지 않았다** — 실측 2건은 둘 다 **난방·히트펌프 블록 내 라인분리**로 찾은 것이라(윤성호 0114 난방설비 내 축열조 90ton · 임미라 히트펌프 블록 내 축열조탱크 FRP보온 20톤) **같은 방식의 전수 점검이 필요하다**. **판정하지 않는다**.",
...
#     ⚠️ 다만 **우민재 239,841원/㎡는 필름 밴드 상한 240,000에 158원 차로 붙어
...
#     상한 240,000과의 간격이 158원 → **42원**으로 줄어든다(🔴153차 정정 — 종전 159는 반올림). ⚠️`ACTUALS` 값
...
#   📒 2026-09-15 131차 — 확인요망 **문자열 수는 항목 수가 아니다**
#     130차가 *"전체 50건 / 이 상수 15건"*이라 적었는데 131차에 다시 세니 **53 / 18**이다.
#     🔴 늘어난 3건은 **130차가 대장을 만들며 이 source에 쓴 로그 문장 자체**다 —
#       **대장을 만드는 행위가 집계를 늘린다**(그래서 131차 로그는 대괄호를 안 쓴다).
#     ✅ 이 상수 밖 35건 중 **9건은 이미 닫혔다**: WARRANTY_STATUTORY **4건 전부**
#       (2026-08-18 법제처 원문) · FR_TABLE 1장 54%(**68차 F1 철회**) ·
#       ACTUALS_COUNT 관리동·방풍(**93차 도면 확정**) · GROUND_LOSS_COEF 식 형태(**85차**).
#       **1건은 중복**이다 — TOTAL_PYEONG_PRICE의 비닐 노무비 702−473 vs 228 = **A**.
#     🔴 살아 있는 항목은 **PUMSEM 5 + 그 밖 20 = 25개**이고, **한 항목이 상수 경계를
#       넘는다**(U2 = U_VALUE·U_DESIGN·PERIOD_LOAD_ADJUST_K / CM1 = CAPEX_MAJOR 3종).
#       129·130차 후퇴가 **시간축** 흩어짐이었다면 이것은 **공간축** 흩어짐이다.
#     ⚠️ `grep`으로 세지 않는다 — 닫힌 것·인용·중복·로그가 섞인다.
...
#   자료다 — 혼동해서 재사용하지 않는다.
#   🔴 2026-09-15 132차 — **그 원본 CSV는 리포에 없다(0바이트)**
#     레지스트리 desc가 *"원본은 `소득분석DB/…_20201015.csv`(CP949, 1,213행)에 보존"*
#     이라 적는데 파일은 **빈 blob(e69de29)**이고, `git`상 **이 상수를 '실측'으로
#     승격한 커밋(b652848, 2026-07-23)부터 줄곧** 비어 있었다. 25항목은 세션 안에서
#     전사됐고 원본은 들어오지 않았다.
#     ⚠️ 그래서 위 [확인요망](광열동력비 3010104 ↔ 수도광열비 3010117)의 사유가
#       *"원문에 정의문서가 없다"* → **"원문 자체가 리포에 없다"**로 바뀐다.
#       ★사용자가 CSV를 다시 넣어야 답할 수 있다(data.go.kr ID 15069669).
#     ⚠️ `_load_income_item_rows()`는 **파일 없음과 빈 파일을 구별하지 않는다** —
#       둘 다 `[]`를 돌려준다(132차는 동작을 바꾸지 않았다. 범위 밖이다).
#     📌 이 상수는 `source_refs`가 null이라 **추적성 감사 게이트의 검사 범위 밖**이었다
#       (그래서 게이트는 계속 green이었다). 132차 가드가 **레지스트리 서술이 전체
#       경로로 지목한 리포 파일**(7종)을 대신 본다.
#     📌 값이 틀렸다는 뜻이 아니라 **리포 안에서 대조가 불가능**하다는 뜻이다.
#   근거: `근거_대장3항목_리포내확인_20260915.md`
#   📒 2026-09-15 133차 — 이 상수만의 문제가 아니었다
#     레지스트리 54개 중 **16개가 `source_refs` 0건**이고 그중 **12개가 '실측' 계열**이다.
#     ref가 0건이면 추적성 감사의 **원문 실재 검사가 한 번도 돌지 않는다**(빈 리스트는
#     0회 순회한다) — 검사되는 것은 status가 enum 안에 있는지뿐이고 '실측'은 enum 안에 있다.
#     🔴 패턴이 거꾸로다: **공공기준·법정기준 21개는 전부 ref가 있는데**, 전사 오류가
#       가장 일어나기 쉬운 **'실측' 16개 중 11개(69%)가 0건**이다.
#     ⚠️ 이 OPEX 상수는 **원문이 0바이트**라 (B)계열이지만, `PUMSEM_ITEMS`처럼
#       **원문이 리포에 실재하는데 안 붙은** (A)계열이 5개 있다 — 134차 과제.
...
#   📒 2026-09-15 135차(레드팀 26회차) — 발견 10건·거짓 양성 0건(값 변경 0)
#     ✅ "엔진 상수 값 변경 0"(128~134차)은 **독립 재현됐다** — 008feb9..HEAD 비주석 0줄.
#     🔴 **이번 발견은 성격이 다르다**: 25회차까지는 서술·집계에 몰렸는데
#       이번엔 **가드 자체의 결함 2건**이 나왔다.
#       **F3** 132차 유일성 가드가 `if ws.title == "공종별내역서"`로 **타 시트를 버려**
#         "워크북 전체"라는 주석과 달리 **검사한 적이 없었다** → 전 시트 스캔으로 교체.
#       **F8** CM4 가드의 두 번째 단언이 첫 단언과 **항등식**이라 정보가 0이었다 →
#         "비환급 = 비환급재료 + 노무 전액" 분해로 교체.
#     🔴 **F4 — 대장이 이미 닫힌 것을 살아 있다고 적었다**(129·130차에 이은 **세 번째**).
#       `AC3`(우민재 면적 기준)는 **94차가 이미 확정**했는데 131차가 못 봤다 —
#       이번엔 **한 상수 안에서 바로 다음 문단**을 놓쳤다. 항목 20→19, 닫힘 9→10.
#       📌 원인은 대장이 **`[확인요망]` 문자열을 모집단**으로 삼는 것이다 — 태그 없는
#         미해결(94차가 새로 연 2,323 vs 2,321.87)은 **구조적으로 누락**된다.
#     ⚠️ F1: 132차 CM3 대조군이 **전수가 아니었다**(주석 행은 4개가 아니라 5개이고,
#       주석 없이 금액 0인 행은 08 외에 4개 더 있다 — R10 PO는 114,241,498원 블록이
#       실재하는데 수량 공란으로 빠진다). 결론은 헤지돼 있으나 **근거력이 약해졌다**.
#   근거: `검증절차_레드팀.md` 26회차
#   📒 2026-09-15 136차 — 품셈 PDF를 **프로세스당 한 번만** 연다(값 변경 0)
#     `pumsem_extract.py`의 네 추출 함수가 각각 `PdfReader` + `pdfplumber.open`을
#     따로 불러 **한 프로세스에서 89.8MB PDF를 4회 파싱**했다 → 공유 핸들 `_doc()`로
#     묶었다(`close_doc()`로 명시 해제). **파싱 4회 → 1회**(계수기 실측).
#     ✅ **추출 결과는 비트 단위로 불변**이다 — 7종 `repr()` 다이제스트가 전부 같다
#       (136차 가드가 그 값을 고정한다). 성능을 얻자고 값이 바뀌면 125·129차의
#       64계수 대조가 통째로 흔들리므로 **불변이 이 변경의 본체**다.
#     📊 교차 3회 실측 **19.53s → 15.02s(−23%)**, 3파일 게이트 **44.38s → 35.92s**.
#     ⚠️ 133·135차의 **세그폴트를 고쳤다고 말하지 않는다** — 재현 조건을 잡지 못했고
#       **인과를 확인하지 않았다**. 확실한 것은 파싱 횟수·출력 불변·시간뿐이다.
#     📌 `_system_index()`는 **여전히 맨 앞**이다 — 폰트 없는 기계에서 PDF를 열기 전에
#       `FontsUnavailable`이 올라야 128차 skip 규율이 유지된다.
#   근거: `근거_품셈PDF공유핸들_20260915.md`
#   📒 2026-09-15 137차 — `match` 등급 확정(값 변경 0, 표기만)
#     26회차 F5의 잔여였던 **미기재 56건**에 등급을 매겨 레지스트리 **148건 전부**가
#     등급을 데이터에 기록한다(**exact 90 · partial 50 · near 8**).
#     🔴 등급을 안 적으면 감사기가 `r.get("match", "exact")`로 exact로 잡고
#       build_site 배지에서도 exact와 똑같이 **무배지**가 된다 —
#       **적지 않은 것이 "정확히 일치한다"는 주장으로 바뀐다**.
#     🔴 **near 4건이 이번 수확**이다(값이 원문과 **다르다**):
#       `STRUCTURE_ONLY_PYEONG` 163,400 vs 우민재 계산 **163,468**(146차 정정, A-8 엔진값을
#         이 문서로 근사 확인한 것이지 원문 전사가 아니다) ·
#       `BENCHMARK_BANDS` 필름 상한 240,000 vs **239,842**(올려 잡은 경계, 상한만 뒷받침) ·
#       `ACTUALS_COUNT` 우민재 2건 2,323 vs **2,321.87**(26회차 F4의 **AC5** 그 자체).
#     ⚠️ 산출물이 바뀌었다 — 근거대장 HTML 배지가 **[근접] 4→8 · [부분] 43→50**(+11).
#       값은 그대로이고 **독자가 보는 근거력 표시**가 늘었다.
#   근거: `근거_match등급확정_20260915.md`
#   📒 2026-09-15 138차 — `exact` 39건 원단위 재검산(값 변경 0)
#     137차가 남긴 *"exact가 정말 일치하는지 재계산하지 않았다"*를 집행했다.
#     견적 원문 **39건 대조 → 불일치 0 · 확인 불가 0**(인쇄 확인 24 · 재계산 6 ·
#     note에 금액 없음 9).
#     🔴 **known_total은 문서의 총계가 아니다** — 두 표본에서 같은 구조가 나왔다:
#       우민재 문서 합계행 **453,478,913** + '합계제외' 3행 **2,679,227** = 456,158,140
#       이두희 문서 `계`   **423,454,980** + 영세율 적용 **10,151,480** = 433,606,460
#       (맹주연도 직재+직노+기계경비 3성분 합이고 총액은 인쇄돼 있지 않다)
#       → **인쇄값만으로 정확히 재현되므로 exact는 유지**하되 note에 명시했다.
#     🎯 **우민재의 차이 2,679,227은 CAPEX_MAJOR_UNCLASSIFIED 값과 정확히 같다** —
#       엔진의 미분류가 **문서가 '합계제외'로 표시한 바로 그 집합**임이 드러났다.
#     ⚠️ 재집계 규칙(합계제외·영세율을 포함하는 것)의 타당성은 **회계 판단**이라
#       판정하지 않았다.
#     📌 도중에 **내 판독기를 두 번 고쳤다**: PDF가 숫자를 자간 공백으로 쪼개 렌더해
#       실재 값을 놓쳤고(과소), 공백을 통째로 지웠더니 인접 숫자가 붙었다(과교정).
#       132차 방식(x 간격 4px 결합)으로 교체 — 125차 P1~P3·133·134차와 같은 계열이다.
#   근거: `근거_exact39_원단위재검산_20260915.md`
#   📒 2026-09-15 139차 — 대조 기준 보강 + 재검산기를 리포 도구로(값 변경 0)
#     138차가 *"note에 금액이 없어 대조하지 못했다"*고 남긴 **9건**에 앵커를 적었다.
#     🔴 **기준이 없으면 그 ref는 영원히 검산되지 않는다** — `_ref_ok()`는 파일 실재만
#       보므로 게이트도 그 사실을 모른다. 적기 전에 **원문에서 먼저 찾았다**(8/9 인쇄 확인,
#       1건은 138차가 밝힌 우민재 재집계값).
#     ✅ `verify_refs.py` + `verify_refs_dump.txt` 신설 — 정적 검사는 게이트에서 돌고
#       원문 16종 전수 재검산은 `python verify_refs.py --full`로 돌린다.
#       전수 결과 **OK 32 · RECOMPUTED 7 · MISS 0 · UNK 0**(대조 못 한 건 0).
#     🔴 **변이 2종이 처음에 통과했다** — 가드가 도구를 **신뢰만** 하고 있었다:
#       M3 재집계 표지에 **빈 문자열**을 넣으면 모든 note에 매칭돼 전부 통과 ·
#       M5 `static_check` 무력화는 **앵커 없는 ref가 0건이라 결과로 구별되지 않는다**
#       → **red 자기검증**(합성 입력으로 도구를 시험)을 넣었다. 26회차 F3과 같은 계열이다.
#   근거: `근거_대조기준보강_20260915.md`
#   📒 2026-09-15 140차 — `partial`·`near` 58건에도 대조 기준(값 변경 0)
#     139차가 `exact` 39건을 닫았으나 **나머지 58건은 여전히 "무엇을 보면 되는지"가
#     없었다**. `partial`은 문서가 값의 *일부·해석 근거*일 뿐이라 금액 앵커가
#     성립하지 않는다 → 기준을 **①숫자 ②위치 ③인용 문구** 중 하나로 넓혔다.
#     **기준 없음 24 → 0**(레지스트리 148건 전부가 닫혔다).
#     🔴 **자동 유도를 한 번 접었다**: `CAPEX_MAJOR_EVIDENCE_STATUS`의 상태 문자열을
#       `—`로 쪼개 포함/제외를 가르려 했으나 **주석 안에도 `—`가 있어** `hvac` 10건이
#       **4건으로 오독**됐다 → 자동 분류를 포기하고 사람이 읽는 기준으로 적었다.
#       125차 P1~P3·133·134·138차에 이어 **또 텍스트 자동 분류가 틀렸고**, 이번에는
#       **틀린 결과를 쓰지 않고 접었다**.
#     🔴 그 과정에서 **134차의 일괄 문구가 부정확할 수 있음**이 드러났다 —
#       오기수는 `greenhouse_structure`에 **제외 사유**로 등장한다("근거 표본"이 아니다).
#   근거: `근거_대조기준_partial_near_20260915.md`
#   📒 2026-09-15 141차 — 선언 건수 8종 검수: **8/8 일치**(값 변경 0)
#     🔴 **140차의 "셀 수 없다"는 방법의 문제였다** — 상태 문자열을 `—`로 쪼갤 게
#       아니라 `CAPEX_MAJOR_CASE_CHUNKS`(케이스 15 × 카테고리 금액)에서
#       **"금액>0 케이스 수"를 세면** 문자열을 건드릴 필요가 없다.
#       125차 P1~P3 계열과 다르다 — **파서를 고칠 게 아니라 쓰지 말았어야** 했다.
#     ✅ 선언 vs 실데이터 8/8 일치 · **잔차(분류합+미분류=known_total)도 15케이스 전부** 일치.
#     📌 세는 단위는 **파일이 아니라 케이스**다 — `백가은·조윤정`은 **쌍 견적 통합 1건**
#       (140차가 이름을 세어 15 vs 14로 어긋난 이유가 이것이다).
#     🔴 **서술 비대칭 2건**: 8종 중 6종은 "나머지가 왜 0인지"를 적는데
#       `auxiliary_facility`(미언급 14건)·`thermal_storage_insulation`(13건)은 적지 않는다.
#       0인 이유는 **판단**이라 데이터에서 유도되지 않아 **값을 고치지 않았다**.
#     ⚠️ **건수가 맞는 것과 귀속이 옳은 것은 다르다** — hvac 10건에 최선동이 들어 있으나
#       안개분무시설 18,500,000의 귀속은 여전히 대장 `CM1`의 `[확인요망]`이다.
#   근거: `근거_선언건수_검수_20260915.md`
#   🔴 2026-09-15 142차 — **112차 이후 첫 엔진 상수 값 변경**(서술 필드 말미 추가)
#     141차가 남긴 비대칭 2건(`auxiliary_facility`·`thermal_storage_insulation`)의
#     0 사유를 채웠다. **판단이 아니라 리포가 이미 기록한 사실**이다:
#       이준희 — 재배실 4,756.32 + **관리실 648㎡**가 실재하나 '0402 재배실벽체 및
#                관리동지붕공사'가 온실구조에 **혼입**돼 0(52차)
#       강정구 — 부속자재에 관리동 판넬·출입문 부속 약 5,597,384원 포함,
#                **독립 공종이 아니라** 분리 안 함(56차)
#       오기수 — 관리동커텐 2,817,269원이 **자동개폐로 귀속**(57차)
#     🔴 따라서 *"실측 1건"*은 **"부대시설이 1건뿐"이라는 뜻이 아니다**.
#       ⚠️ 나머지 11건의 0 사유는 **확인되지 않았다 — 판정하지 않는다**.
#     ⚠️ **영향 범위**: `build_site.py:136`이 **접두만**(`startswith`) 읽으므로
#       태그는 실측/추정 **불변**이고, 이 값은 **계산에 쓰이지 않는다**.
#       엔진 리터럴이 **묵시적 문자열 연결**(5줄)이라 1차 치환이 꼬리를 중복시켰다 —
#       연결 전체를 한 줄로 합쳐 고쳤다(드리프트 가드로 확인).
#     🔴 **"언급 ≠ 근거"가 실증됐다**: 이름을 적자 140차의 언급 카테고리 수가
#       이준희 5→6 · 강정구 6→7 · 오기수 5→6으로 늘었다 —
#       **늘어난 언급은 근거가 아니라 0 사유**다.
#   근거: `근거_0사유보강_20260915.md`
#   📒 2026-09-15 143차 — 관리동 실재 전수 점검: **14건 중 13건에 있었다**(수치 불변)
#     142차가 미확인으로 남긴 11건을 **견적 원문 12파일 직접 판독**으로 채웠다:
#       최혁진 관리동 트러스 상현재/하현재 각 1,020,144원 · 이두희 작업동 콘크리트
#       11,887,296원 · 한일그린텍 트러스(작업장) 1,406,856원 · 맹주연 작업장 스크린
#       12,125,520원 · 백가은·조윤정 양액실/관리실 49㎡ 6,370,000원 · 박규현 공종명
#       '(작업동:AL고정스크린)' · 구창회 트러스(작업장) 1,985,596원 · 한수진·최선동
#       작업동바닥콘크리트 · 우민재는 **내역서 0건인데 공사설명서에 작업장**이 있었다.
#     🔴 **그래도 0이 틀린 것은 아니다** — 찾은 항목은 거의 전부 골조·기초·커튼·피복
#       공종 **안의 라인**이고 **독립 부대시설 공종은 윤성호 0116 하나뿐**이라
#       분류 규칙(독립 공종만 편입)상 0이 맞다. **문제는 값이 아니라 읽힘이다.**
#       ⚠️ 금액 이관은 분류 규칙을 바꾸는 일이라 **★사용자 결정**이다(값 불변).
#     ⚠️ **임미라만 확인되지 않았다** — 전 시트 471행·키워드 20종으로 0건이나
#       **리포에 견적서 1종뿐**(도면·설명서 없음)이라 **"없다"고 말할 수 없다**.
#     📌 방법 교훈: **검색 대상을 내역서 1종으로 잡으면 놓친다**(우민재가 그 사례).
#   근거: `근거_관리동_실재점검_20260915.md`
#   📒 2026-09-15 145차 — ★D-10 후보 설계 정정 + 감응 측정(값 변경 0)
#     🔴 **129차 F2의 "품목별 요율값(0/2%/3%/5%)"이 부정확하다**: 원문 [주]를 분모까지
#       읽으면 요율은 **두 계열**이고 **분모가 다르다** —
#         공구손료·경장비 기계경비 = **인력품(노무)의 %** → 2%·3%, **29품목**
#         잡재료·소모재료(실리콘) = **주재료비의 %** → 5%, **2품목**
#         (천장우레탄판넬·샌드위치판넬)
#       온실피복 #4는 ③공구손료 3% + ④잡재료 5%를 **동시에** 갖지만 **모순이 아니다**.
#       → **필드 하나로 담을 수 없다. 두 필드가 필요하다.**
#     🔴 진짜 모순은 **선홈통**(같은 분모에 2%와 3%)뿐이고 **그 감응은 0.0059%p**다 —
#       113차부터 끌어온 그 [확인요망]은 **집계 수준에서 결정을 막지 않는다**.
#     📊 감응: 공구손료 규정 품목이 **인·일 계수의 66.5%**, 가중 평균 **노무비의 1.98%**.
#       ⚠️ 노임단가가 시세성이라 **금액은 산출하지 않았다** — 비율까지가 한계다.
#   근거: `근거_감응측정_D10_D12_20260915.md`
# ─────────────────────────────────────────────────────────────
@dataclass
class OpexItemCode:
