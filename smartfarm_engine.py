...
CAPEX_MAJOR_EVIDENCE_STATUS = {
    ...
    # 이후 표본 추가 차수는 이 건수도 같은 차수에 동기한다(52차 이준희 반영 완료 — 6표본).
    "greenhouse_structure": "실측(6건: 우민재·최혁진·이두희·윤성호·한일그린텍·이준희)",
    "auto_opening_system": "실측(6건: 우민재·최혁진·이두희·윤성호·한일그린텍·이준희 — 보온커튼·스크린은 구동부와 함께 "
                           "이 카테고리 소속으로 확정, 2026-08-17 사용자 결정(우민재 방식 통일). "
                           "종전 표기 '모터·보온재 미분리'는 문제가 아니라 관례로 승격)",
    "hvac": "실측(4건: 우민재·최혁진·윤성호·이준희[유동휀만 — 난방설비는 '본 공사제외'] — 이두희·한일그린텍은 "
            "견적 범위에 난방설비 자체가 없어 0. 한일그린텍은 유동휀·배출환풍기가 실재하나 '환급금 재투자' "
            "블록이라 직접공사비 분모 밖)",
    "irrigation_fertigation": "실측(6건: 우민재·최혁진·이두희·윤성호·한일그린텍·이준희)",
    "ict_control": "실측(4건: 최혁진·이두희·윤성호·이준희 — 우민재·한일그린텍은 독립 환경제어시스템 라인 없음(0). "
                   "한일그린텍 하우스콘트롤 7,434,000원은 개폐 접점 제어반이라 자동개폐 소속. 이준희 "
                   "83,463,440원은 표본 최대 관측치[벤로형 유리 사양])",
    "electrical": "실측(3건: 최혁진·윤성호·이준희 — 우민재·이두희·한일그린텍은 별도 전기공사 공종 없음(0), "
                  "전선류는 각 공종 내 산재로 집계 불가)",
    "auxiliary_facility": "실측(1건: 윤성호 '0116 관리사(사무동)공사' 40,093,200원 — 독립 공종, 2026-08-17 P2-17)",
    "thermal_storage_insulation": "부분실측(축열탱크만 1건: 윤성호 축열조 90ton 41,958,000원 라인분리, 2026-08-17 P2-17. "
    ...
    "design_supervision_fee": "참고요율(법정기준) — 「공공발주사업에 대한 건축사의 업무범위와 대가기준」(국토교통부고시 "
                              ...
                              "불산입, 값은 0 유지(종별 선택·실계약액은 판단성·시세성). 개별 케이스 실지출 대조는 아직 없음",
...
CAPEX_MAJOR_CASE_CHUNKS = {
    ...
    # 5,404.32㎡=재배실 4,756.32+관리실 648). 표본 최초의 벤로형 유리온실.
    ...
    # 원문이 양액시설 하위로 편성한 소속을 따름), 1209 복합환경제어시스템
    # 83,463,440→ict_control(최혁진·윤성호 0113 '환경제어시스템' 명시 라인 관례 —
    # 표본 최대 관측치, 기존 3건 17.9~26.2M의 3~4.7배: 벤로형 유리온실 사양 신호),
    # 13 동력간선→electrical(윤성호 0115 선례).
    ...
    # 원본 관찰 2건([확인요망] 계열): ①08 예인형 개폐장치공사가 집계표에 금액
    # 미반영인데 내역서엔 #REF! 파손 항목(개폐모터 단가·합계)과 실항목(보온다겹
    # 스크린 6,050,000·노무 소계 8,603,500 등)이 잔존 — 미채택 대안인지 수식
    # 파손 누락인지 원문만으로 확정 불가(발주 전 확인 필요, 군산 '행 총액 공란'
    # 계열의 원본 결함 신호). ②0401 재배실지붕공사(PO)는 단가만 있고 금액 0 —
    # 유리 채택·PO 대안 미채택 흔적(비교 견적 잔재).
    "이준희": {
        ...
        "hvac": 7093767,                     # 09유동휀 설치 공사 전액(난방설비 11은 "본 공사제외" — 별도 발주)
        "irrigation_fertigation": 66934911,  # 1201기계실(28,952,319)+1203관수(28,859,014)+1204배수(3,904,157)+1207CO2배관(5,219,421)
        "ict_control": 83463440,             # 1209복합환경제어시스템 전액 — 표본 최대 관측치(벤로형 유리 사양)
...
#   계약·정산 확인 시.
#   ── ACTUALS 원문 대조 현황(40차 기준) ──
#   원문 대조 완료 4건: 이두희·최혁진·우민재(P1-7, 확정액 계열)+한일그린텍(40차,
#     설계예산 — 원단위 일치). 근접 대조 1건: 공주장원리(이동혁 견적 448,000,000
