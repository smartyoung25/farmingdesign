...
#   완충은 `safety`(기본 1.1)뿐이고 그 값의 출처도 여전히 미상이다(14절 B3).
...
#     **해소는 아니다** — 나머지 차이는 u_design(5.7) vs u_period(2.66)의 이원화다[확인요망].
# ─────────────────────────────────────────────────────────────
...
#   PDF p.378 / 인쇄 342 전사(그룹 5 + 계). 값은 (관류, 틈새환기, 지중) 평균 %.
...
#     누락은 틈새환기 8.6% + 지중 −0.5% = **+8.1%** — 즉 엔진 최대난방부하는 약 8%
#     **과소**다(84차가 "방향은 과소산정"이라 적은 것의 크기가 이제 나온다).
...
#   E:\이암허브\...\202201_스마트팜 표준화_품셈.pdf) 제7장(원문 p.138~162[PDF↔인쇄 오프셋 미검증 — 리포 밖 파일]
...
#   원문 제3절 말미(p.163~164[오프셋 미검증])의 <그림7-18/7-19> 온실공사품셈 활용
...
PUMSEM_ITEMS: list[PumsemItem] = [
    ...
    PumsemItem("행잉거터공사", "그라운드커버", "㎡",
               {"보통인부": 0.004}, {}),
    # 경량철골비닐온실공사(제3절, 원문 p.160~162[오프셋 미검증]) — 유리온실과 구조체계가
