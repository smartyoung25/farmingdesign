...
def heating_load(surface_area_m2: float, cover: str, t_target: float,
    ...
    ✅ 식 원출처 자구 확보(2026-09-13 77차, 사용자 지시 "최대부하 산정 커튼 전제 진행"):
      시설평가/20230627_스마트팜 시설의 구조와 이해_김평화(제공).pdf **printed p.49** —
    ...
    🔴 그러나 u_design에 무엇을 넣어야 하는가는 **미해결이고, 현행이 과대일 가능성이
      ...
      리포 안 다른 문서에 있었다 —
        스마트팜연구DB/온실열손실저감및차단기술연구.pdf **printed p.12**:
    ...
    #   ⚠️ 남은 미확보(17회차 F4로 전파): ①safety(=HEATING_SAFETY_FACTOR 1.1) 자체의 출처는
    #   여전히 미상이고 같은 문서 p.53이 수치편을 "별도 자료"로 분리했다. ②**국내 등유
...
#     printed p.350~351 **[표 3-3-38] TAC 위험율별 온실의 난방설계 기온(℃)**
#     printed p.359~360 **[표 3-3-42] 온실설계용 설정온도별 난방디그리아워(10³℃·h)**
...
CAPEX_MAJOR_EVIDENCE_STATUS = {
    ...
    "greenhouse_structure": "실측(11건: 우민재·최혁진·이두희·윤성호·한일그린텍·이준희·맹주연·강정구·백가은·조윤정·박규현·구창회 — 오기수는 설비 전용 "
                            "부분 범위 견적이라 골조·피복 품목 전무(0))",
    "auto_opening_system": "실측(12건: 전 표본 — 보온커튼·스크린은 구동부와 함께 "
                           "이 카테고리 소속으로 확정, 2026-08-17 사용자 결정(우민재 방식 통일). "
                           "종전 표기 '모터·보온재 미분리'는 문제가 아니라 관례로 승격. 박규현 170,837,702는 마그마 "
                           "동력피복제어기 24,000,000 포함 — 구동 제어반 계열 최대 관측)",
    "hvac": "실측(7건: 우민재·최혁진·윤성호·이준희[유동휀만 — 난방설비는 '본 공사제외']·맹주연[유동팬 42대·환풍기 "
            ...
            "유동휀 24대] — 이두희·한일그린텍·강정구·오기수·구창회는 견적 범위에 난방설비 자체가 없어 0. "
            "한일그린텍·구창회(동일 업체 1·2호 현장)는 유동휀·환풍기가 실재하나 '환급금 재투자' 블록이라 직접공사비 분모 밖, "
            "강정구·오기수는 부분 범위 견적[각각 골조 쪽만·설비 쪽만 — 오기수는 유동휀·배기휀 '단자'만 예비])",
    "irrigation_fertigation": "실측(11건: 우민재·최혁진·이두희·윤성호·한일그린텍·이준희·맹주연·오기수·백가은·조윤정·박규현·구창회 — 강정구는 부분 범위 "
                              "견적이라 관수·양액 품목 전무(0))",
    "ict_control": "실측(5건: 최혁진·이두희·윤성호·이준희·오기수 — 우민재·한일그린텍·맹주연·강정구·백가은·조윤정·박규현·구창회는 독립 환경제어시스템 라인 없음(0) — 구창회 하우스콘트롤 7,153,738원은 1호 7,434,000원 동계열 개폐 접점 제어반(감응 1.89%p): "
    ...
    "electrical": "실측(4건: 최혁진·윤성호·이준희·박규현[전기 공종 잔여 2,832,665 — 케이블·잡자재·인건비, 공종의 제어기·팬은 "
                  "실체 분리] — 우민재·이두희·한일그린텍·맹주연·강정구·오기수·백가은·조윤정·구창회는 별도 전기공사 공종 없음(0), "
