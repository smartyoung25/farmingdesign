...
# 🔴 153차(레드팀 28회차 [7] 재검증) — `_LOC`에 **제어문자 U+0008이 3개** 박혀 있었다.
#   140차에 이 패턴을 쓸 때 `\b`(단어 경계)가 **백스페이스로 변환**돼 들어갔고,
#   그래서 `[A-Z]{1,2}\d{1,4}`(엑셀 셀 참조)·`행`·`열` **세 분기가 죽어 있었다**.
#   soft ref 61건 중 **21건**의 「위치」 판정이 과소 인식됐다(게이트는 다른 기준이 있어 통과).
#   146차 `_ROUND`·148차 `NUM`에 이은 **같은 버그의 세 번째**다 —
#   **정규식을 쓸 때는 `.pattern`을 눈으로 확인한다**(리터럴만 보면 안 보인다).
...
_LOC = re.compile(r"(p\.?\s?\d+|인쇄\s?p|쪽|페이지|\[표[^\]]*\]|<표[^>]*>|<그림[^>]*>|별표"
                  r"|시트|[A-Z]{1,2}\d{1,4}\b|제\s?\d+\s?[절장조항호]|슬라이드|r\d{2,}"
                  r"|행\b|열\b|목차|부록|§\d|머리말)")
