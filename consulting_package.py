...
#    「적격」0 · 「공급사」0회.
FUNCTION_GAPS = (
    ("F2", "KCS 형식 시방", "엔진 원문에 「KCS」 0회 — 형식 변환이 없다"),
    ("F2", "ICT 기자재 KS/SPS/TTAS 사양",
     "표준 번호 체계를 엔진이 모른다(D19는 대조만 한다)"),
    ("F3", "수의계약 한도 판정",
     "지방계약법 시행령 한도는 **법정값**이다 — 원문 확보·레지스트리 등재가 먼저다(★)"),
    ("F3", "나라장터 등록 지원", "엔진 원문에 「나라장터」 0회"),
    ("F3", "하자이행보증 2%·1년",
     "`warranty_period`는 기간만 낸다 — 보증 요율은 **법정·계약값**이라 미등재(★)"),
    ("F3", "적격 공급사 풀", "업체 **선정**은 1절이 금한 판단성이다 — 풀 자체가 없다"),
    ("F3", "2인 이상 견적 자동 수집",
     "`compare_quotes`는 **받은 견적을 비교**할 뿐 수집하지 않는다"),
)
...
def function_index() -> dict:
    ...
    for fn, name, why in FUNCTION_GAPS:
        gaps[fn].append({"name": name, "why": why})
