...
def test_traceability_audit_gate_green_and_backlog_pinned():
    ...
    # 73차: 36→37 — PYEONG_TO_M2 등재(refs 48→49). 17회차 F1이 정정한 대로
    #       이것이 "마지막"은 아니었다.
    ...
    # 🔴174차: 56/159 → 57/162(P3 사후관리 — 시방서 하자보수 조항 1상수)
    # 🔴175차: 57/162 → 60/171(P2 인허가 2상수 + P4 재료승인 첨부 1상수)
    # 🔴176차: 60/171 → 61/172(농진청·세법 내용연수 67품목)
    # 🔴177차: refs 172 → 173(농진청 원문 확보 — 값은 등재하지 않았다)
    # 🔴178차: refs 173 → 174(OPEX 비목 대조 — 추적성 사각 3 → 2)
    assert a["counts"]["registry_constants"] == 65 and a["counts"]["source_refs"] == 178
