...
- 판단성 값(작목·부지·업체 선정, 최종판정)은 초안·참고까지만. 판정·추천 자동화 금지
  - 단서(★사용자 결정 2026-09-21, 190차): **규칙 적용형 등급·판정은 예외**다. 네 조건을
    모두 지킬 때만 — ①규칙·임계값이 레지스트리에 status 「결정」으로 등재되고 ②같은 입력이면
    같은 결과이며(난수·시각 의존 0) ③등급·판정마다 **항목별 근거 행**을 함께 내고 ④임계값을
    주입으로 덮어쓸 수 있고 반환이 **적용된 규칙**을 드러낸다. 미검증을 통과로 세지 않는다.
    막던 것은 **근거 없는 판정**이지 규칙 적용이 아니었다.
...
## 작업 관례
- 과제 1건 = 차수 1개 = 커밋 1개. 차수로그.md "갱신 이력" 맨 위에 항목 추가(검증한 것/안 한 것 명시)
  + 작업지시서 헤더 "최종 갱신" 한 줄 교체(204차 분리 — 착수점은 작업지시서, 이력은 차수로그)
...
- smartfarm_engine.py를 계층별로 쪼개지 않는다 — 엔진 분리 금지(★사용자 결정 2026-09-22, 191차). 등급·보증 판정 함수가 엔진 안에 있는 것은
  설계다. 떼어 내면 판정 모듈이 엔진을 되부르거나 설계값을 복제해 제2의 계산 출처가 된다(1절 첫 줄 위반).
  경계는 파일이 아니라 호출 방향으로 지킨다 — 계산 함수는 판정 함수를 부르지 않고,
  판정 함수는 계산 함수를 부르지 않는다(판정 입력은 인자 주입). 계산값을 판정에 넣는 것은
  조립 계층 consulting_package.py의 일이다
