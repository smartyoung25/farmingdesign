...
def test_146cha_redteam27_corrections_hold():
    ...
    **절대표현**(유일한·전량·0건)과 **문자열 탐색으로 감응을 재는 습관**이다.
    ...
    assert len(stars) == 44, (
        f"엔진의 ★ 줄이 {len(stars)}개다 — 222차 전수는 44개다(+5 — ★2022-104호 준거 확정이 D-15를 닫았다). "
...
def test_166cha_service_design_claims_are_measured():
    ...
    # 🔴175차 — P2·P4까지 구현돼 **백로그가 전부 닫혔다**. 남은 제안은 0건이다.
    PROPOSED = set()
    ...
    # 🔴175차 — P2로 「지내력」·「농지전용」이 확인 항목으로 코드에 들어왔다
    for kw in ("유지보수", "측량"):
        assert blob.count(kw) == 0, (
            f"🔴 「{kw}」가 코드 5파일에 {blob.count(kw)}건 있다 — 문서는 0건이라 적는다. "
            "구현이 들어왔다면 커버리지 표와 7절 백로그를 갱신하라")
    # 반대 방향 — 구현된 것은 **있어야 한다**
    for kw in ("검측", "시운전", "준공", "정기점검", "지내력", "농지전용"):
        assert blob.count(kw) > 0, (
            f"🔴 「{kw}」가 코드에서 사라졌다 — 173차 P1·174차 P3·175차 P2 구현이 "
            "되돌아갔다")
...
def test_169cha_critique_numbers_are_recomputed_not_asserted():
    ...
    #    131·134·137·164차 「세는 문자열」 계열의 함정이다. **고유 함수만** 센다.
    CROSS = {"select_specs", "cover_assembly_lookup", "verify_heating_vs_actual",
             "doc_consistency_check"}
    own = list(counts)
    own[2] = len(names_in(blocks[2]) - CROSS)
    assert own[2] == 5, (
        f"🔴 감리 고유 함수가 {own[2]}개다 — 173차 실측은 5개다"
        "(inspection_checklist·commissioning_plan·completion_docset·"
        "design_supervision_fee_reference·warranty_period)")
    total = sum(own)
    front = own[0] + own[3]
    back = own[2] + own[5]
    ...
    assert "백로그가 전부 닫혔다" in design, (
        "🔴 백로그 완료 표기가 사라졌다 — P1·P2·P3·P4가 전부 구현됐다")
...
def test_173cha_inspection_is_transcribed_not_invented():
    """173차 P1 — 검측 기준이 **원문에서 왔는지**, 창작이 섞이지 않았는지.

    167차는 백로그에 *"검측 기준 원문이 리포에 있어야 한다 — 자료 확인 필요"*라 적고
    **재지 않았다**(확인한 것은 코드 5파일 키워드 0건뿐이다). 다시 재니 9축 인덱스에
    **455청크·36종**이 있었고 공사시방서 3종에서 절차 9단계를 전사했다.

    🔴 이 가드는 **문구가 원문에 실제로 있는지**를 hwp에서 다시 읽어 대조한다 —
    등재된 서술을 믿지 않는다(161차 교훈).
    """
    ...
    doc = open(_o.path.join(repo, "근거_P1감리_시방서전사_20260920.md"),
               encoding="utf-8").read()

    # ── ① 절차가 9단계이고 사본 수를 달고 있는가 ─────────────────────
    steps = e.INSPECTION_PROCEDURE_SPEC
    assert len(steps) == 9, f"🔴 검측 절차가 {len(steps)}단계다 — 173차 전사는 9단계다"
    for st in steps:
        assert set(st) >= {"clause", "item", "timing", "actor", "action", "copies"}
        assert st["copies"] in (2, 3), (
            f"🔴 {st['item']}의 copies가 {st['copies']}다 — 사본은 3종이고 "
            "확인된 수는 2 또는 3이다")
    assert sum(1 for x in steps if x["copies"] == 3) == 6, (
        "🔴 3/3 사본에서 확인된 단계가 6개가 아니다 — 교차 확인 수가 바뀌었다")

    # ── ② 🔴 문구가 **원문 hwp에 실제로 있는가**(서술을 믿지 않는다) ──
    _p.importorskip("olefile")
    import chunking_lib_v2 as _c
    base = ("스마트팜스펙/시방서(RFQ), 견적서(세부내역서QOM), 도면(설계도서)/")
    kh = _o.path.join(repo, base + "기후변화 대응 경주형 연동하우스 보급 시범사업 "
                                   "온실공사 김해농원/시방서.hwp")
    assert _o.path.isfile(kh), "🔴 시방서 원문이 사라졌다 — 절차의 근거가 없어진다"
    txt = "".join(_c.hwp_extract_text(kh)["text"].split())
    for frag in ("K.S규격품은검사를생략할수있다",
                 "감독자의입회하에채취하고봉인",
                 "독립된공인기관",
                 "준공검사원제출7일전",
                 "시운전을요하는부분",
                 "각종시험계획서", "각종시험관계철"):
        assert frag in txt, (
            f"🔴 「{frag}」를 원문에서 읽을 수 없다 — 전사가 원문과 어긋났거나 "
            "사본이 바뀌었다")
    # 전사한 값이 원문 문구를 담고 있는가
    joined = "".join(x["action"] for x in steps).replace(" ", "")
    for frag in ("K.S규격품은검사를생략할수있다", "독립된공인기관",
                 "시공자비용으로시행한다"):
        assert frag.replace(" ", "") in joined or frag in joined, (
            f"🔴 전사에서 「{frag}」가 빠졌다")

    # ── ③ 준공 서류는 원문 열거만 담는가(늘리지 않았는가) ────────────
    assert e.COMPLETION_DOCSET_SPEC == ("각종 시험계획서", "각종 시험관계철", "준공도",
                                        "유지관리지침서", "사용검사 신청서"), (
        "🔴 준공 서류 목록이 바뀌었다 — 원문이 열거한 것만 담기로 했다")

    # ── ④ 🔴 판정하지 않는가 ─────────────────────────────────────────
    r = e.inspection_checklist("온실설치")
    assert r["warranty"]["years"] == 2, "온실설치 하자담보 2년(건산법 별표4 제19호)"
    assert len(r["engine_cross_checks"]) == 4
    assert "판정하지 않는다" in r["note"] and "계약문서 기반 품질 보증" in r["note"]
    r2 = e.inspection_checklist("없는공종")
    assert r2["warranty"] is None and "⚠️" in r2["note"], (
        "🔴 미등록 공종에서 하자기간을 지어내거나 조용히 넘어갔다")

    c = e.commissioning_plan(["온풍난방기", "복합환경제어기"],
                             acceptance_criteria={"온풍난방기": "설계 부하 이상"})
    assert c["needs_criteria"] == ["복합환경제어기"], (
        "🔴 기준이 주입되지 않은 장비를 드러내지 않는다 — 이 함수는 채우지 않고 **밝힌다**")
    assert "시공자" in c["cost_bearer"]
    with _p.raises(ValueError):
        e.commissioning_plan([])

    d = e.completion_docset()
    assert d["four_axis"] is None and len(d["items"]) == 5
    assert "준공 가부를 판정하지 않는다" in d["note"]

    # ── ⑤ 레지스트리에 등재됐고 외부 고시 번호는 새지 않았는가 ───────
    reg_raw = open(_o.path.join(repo, "엔진데이터_레지스트리.json"), encoding="utf-8").read()
    reg = _j.loads(reg_raw)
    for k in ("INSPECTION_PROCEDURE_SPEC", "COMPLETION_DOCSET_SPEC"):
        ent = reg["constants"][k]
        assert ent["status"] == "실측" and len(ent["source_refs"]) >= 2, (
            f"🔴 {k}이 등재에서 빠졌거나 ref가 줄었다 — 사각을 새로 만들면 안 된다")
    assert "제2025-311호" not in reg_raw, (
        "🔴 외부 고시 번호가 레지스트리에 들어왔다 — 167차 경계(외부 기준은 문서 참고 "
        "계층에만)를 지켜야 한다")

    # ── ⑥ 선행조건이 이미 충족돼 있었다는 기록 ───────────────────────
    assert "파일명 전수(스코프 제외 적용) | **0건**" in doc and "455청크 · 36종" in doc
    assert "코드에 없다는 것과 리포에 없다는 것은 다르다" in doc
    # 하지 않은 것
...
def test_175cha_permit_dispute_kept_and_equipment_not_judged():
    """175차 P2·P4 — 이견은 **분리**하고, 기자재는 **판정하지 않는다**.

    🔴 인허가 비용 부담이 **사본마다 다르다** — A·B는 「관공서 수속은 발주자 부담,
    민원 처리는 시공자 비용」, C는 「건축주와 협의하여 발주자 부담」. CLAUDE.md
    사실성(*"자료 간 모순은 본문에 섞지 않고 이견 항목으로 분리"*)에 따라 별도 상수로 뒀다.

    🔴 기자재는 시방서가 *「K.S규격에 합격한 신품 … 부득이한 경우 **감독자가 인정하는
    동등품 이상**」*이라 적으므로 **적합 판정을 하지 않는다** — 3열 대조표까지다.
    """
    import os as _o, sys as _s, json as _j, pytest as _p
    repo = _o.path.dirname(_o.path.abspath(__file__))
    if repo not in _s.path:
    ...
    doc = open(_o.path.join(repo, "근거_P2P4_인허가이견_기자재대조_20260920.md"),
               encoding="utf-8").read()

    # ── ① 이견이 **본문과 분리돼** 있는가 ────────────────────────────
    assert len(e.PERMIT_COST_DISPUTE) == 2, (
        "🔴 비용 부담 이견이 2건이 아니다 — 사본 A·B와 C가 서로 다르게 적는다")
    srcs = [x["source"] for x in e.PERMIT_COST_DISPUTE]
    assert any("A·B" in x for x in srcs) and any("C" in x for x in srcs)
    joined = "".join(x["text"] for x in e.PERMIT_COST_DISPUTE)
    assert "시공자의 비용" in joined and "발주자 부담" in joined, (
        "🔴 이견의 두 편 중 하나가 사라졌다 — 둘 다 있어야 이견이다")
    steps_txt = "".join(x["action"] for x in e.PERMIT_PROCEDURE_SPEC)
    assert "비용" not in steps_txt, (
        "🔴 비용 부담이 절차 본문에 섞였다 — 모순은 **이견으로 분리**해야 한다")

    # ── ② 절차 4단계와 사본 수 ───────────────────────────────────────
    st = e.PERMIT_PROCEDURE_SPEC
    assert len(st) == 4 and sum(1 for x in st if x["copies"] == 3) == 3, (
        "🔴 인허가 절차가 4단계·3/3 셋이 아니다")
    assert any(x["timing"] == "착공 3일 이내" for x in st)
    assert any("접수 후 1주" in x["timing"] for x in st)

    # ── ③ 🔴 해당 여부를 **판정하지 않는가** ─────────────────────────
    r = e.site_permit_checklist(area_m2=3000, cover="유리")
    assert r["missing_inputs"] == ["용도지역"], (
        f"🔴 미입력 항목이 {r['missing_inputs']}다 — 주어지지 않은 것만 담아야 한다")
    assert len(r["confirmations"]) == 4
    for c in r["confirmations"]:
        assert c["decided_by"] in ("지자체", "발주자·설계자"), (
            f"🔴 {c['item']}의 결정 주체가 엔진이 됐다")
    assert "판정하지 않는다" in r["note"] and "이견으로 분리" in r["note"]
    assert r["given"]["cover"] == "유리"
    blob = open(_o.path.join(repo, "smartfarm_engine.py"), encoding="utf-8").read()
    # 🔴 1차 설계가 「별표4」를 썼다가 **하자담보 법정 근거**(건산법 시행령 별표4)에
    #    걸렸다 — 131·134·137·164·167차 「세는 문자열」 계열이다. 구별되는 토큰으로 잡는다.
    #    ⚠️「축조신고」·「가설건축물」은 **확인 항목의 이름**이라 엔진에 있어야 하고,
    #    「FRP」는 피복재다. 막아야 하는 것은 **조문 기준 수치**뿐이다.
    for tok in ("100㎡", "건축법 시행령"):
        assert tok not in blob, (
            f"🔴 개별 인허가 조문 기준 「{tok}」이 엔진에 들어왔다 — 원문 미확보다"
            "(웹 요약까지). 해당 여부는 지자체가 정한다")
    assert "가설건축물 축조신고 해당 여부" in blob, (
        "🔴 확인 항목에서 축조신고가 빠졌다 — 이름은 있어야 한다(기준 수치만 없다)")
    assert "건축법 시행령 제15조⑤" in doc, "🔴 외부 조문 출처가 문서에서 사라졌다"
    r0 = e.site_permit_checklist()
    assert len(r0["missing_inputs"]) == 3
    with _p.raises(ValueError):
        e.site_permit_checklist(area_m2=-1)

    # ── ④ 기자재 3열 대조 — 판정 없음 ────────────────────────────────
    assert e.MATERIAL_APPROVAL_ATTACHMENTS == (
        "제조업자 시방서", "시험성적표", "표준 색상철", "카탈로그", "계산서",
        "자재유지관리 지침서"), "🔴 재료승인 첨부 6종이 바뀌었다 — 원문 열거다"
    q = e.equipment_reconcile(["온풍난방기", "환경제어기"],
                              ks_declared={"온풍난방기": "KS 동등품 이상"},
                              attachments_by_model={"온풍난방기": ["제조업자 시방서",
                                                                "시험성적표"]})
    by = {x["model"]: x for x in q["rows"]}
    assert by["온풍난방기"]["ks_declared"] == "KS 동등품 이상"
    assert len(by["온풍난방기"]["attachments_missing"]) == 4
    assert q["needs_ks_declaration"] == ["환경제어기"], (
        "🔴 규격 미선언을 드러내지 않는다 — 채우지 말고 **밝혀야** 한다")
    assert "적합 판정을 하지 않는다" in q["note"]
    assert "감독자가 인정하는 동등품 이상" in q["note"]
    for x in q["rows"]:
        assert "적합" not in x["status"] and "부적합" not in x["status"], (
            "🔴 행 상태가 적합/부적합을 말한다 — 판정은 감독자 몫이다")
    with _p.raises(ValueError):
        e.equipment_reconcile([])

    # ── ⑤ 🔴 두 등재 목록이 다르다는 사실 ────────────────────────────
    assert by["온풍난방기"]["service_life_years"] == 11
    assert by["온풍난방기"]["in_equipment_db"] is False, (
        "🔴 온풍난방기가 기자재 DB에 들어왔다 — 두 등재 목록이 통합됐다면 "
        "`not_in_equipment_db`의 뜻(등재 범위 밖)을 다시 써야 한다")
    assert by["환경제어기"]["in_equipment_db"] is True
    assert "등재 범위 밖" in q["note"]
    assert "두 등재가 서로 다른 목록이다" in doc

    # ── ⑥ 등재됐는가 ─────────────────────────────────────────────────
    reg = _j.loads(open(_o.path.join(repo, "엔진데이터_레지스트리.json"),
                        encoding="utf-8").read())
    for k in ("PERMIT_PROCEDURE_SPEC", "PERMIT_COST_DISPUTE",
              "MATERIAL_APPROVAL_ATTACHMENTS"):
        ent = reg["constants"][k]
        assert ent["status"] == "실측" and len(ent["source_refs"]) == 3, (
            f"🔴 {k}이 등재에서 빠졌거나 ref가 3건(사본 3종)이 아니다")

    # ── ⑦ 하지 않은 것 ───────────────────────────────────────────────
    assert "인허가 비용 부담을 정하지 않았다" in doc
...
def test_198cha_code_pointers_to_documents_resolve():
    ...
    **49를 보고하지 않는다**(159차 교훈).
    ...
    assert len(cited) == 46, (
        f"🔴 비-테스트 코드의 `.md` 인용이 {len(cited)}종이다 — 198차 실측은 44종 · 211차 45종 · 212차 46종(`consulting_package.py`가 외부수집 근거 문서를 인용). "
